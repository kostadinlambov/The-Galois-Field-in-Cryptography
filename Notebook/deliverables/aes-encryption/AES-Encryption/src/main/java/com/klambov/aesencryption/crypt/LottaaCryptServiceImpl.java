 ...
 */
@Service
public class LottaaCryptServiceImpl implements LottaaCryptService {

  public static final String CRYPT_PREFIX = "$[[lottaaCrypt]]$";
  ...
  @Autowired
  private LottaaCryptConfigProperties encryptionConfiguration;

  public LottaaCryptServiceImpl() {
  }

  @PostConstruct
  public void initializeSecretKey() throws LottaaCryptException {
    if (encryptionConfiguration.getKey() == null || encryptionConfiguration.getKey().isBlank()) {
      throw new LottaaCryptException(
          "Encryption Key is missing. Please define a 'lottaa.crypt.key' property with an encryption key as value"
    ...
    try {
      ...
      encryptionKey = Base64.getDecoder().decode(encryptionConfiguration.getKey());
    } catch (Exception e) {
      throw new LottaaCryptException("Encryption Key must be a valid Base64-String");
    }

    if (encryptionKey.length != 16 && encryptionKey.length != 24 && encryptionKey.length != 32) {
      throw new LottaaCryptException("Encryption Key length must be 16, 24 or 32 bytes");
    }
  ...
  @Override
  public String encryptStringData(String dataToEncrypt) throws LottaaCryptException {
  ...
  private String encryptBytesData(byte[] dataToEncrypt, byte[] associatedData)
      throws LottaaCryptException {
    ...
    try {
      ...
      // Encode the cipherMessage to Base64-String and prepend the CRYPT_PREFIX
      return CRYPT_PREFIX + Base64.getEncoder().encodeToString(cipherMessage);

    } catch (Exception exc) {
      throw new LottaaCryptException("Could not encrypt the data", exc);
    } finally {
  ...
  @Override
  public String decryptBase64StringData(String dataToDecryptB64) throws LottaaCryptException {
  ...
  private String decryptBytesData(byte[] dataToDecryptBytes, byte[] associatedData)
    ...
    try {
      ...
      if (ivLength != 12) {
        throw new LottaaCryptException("Unexpected Initialization Vector length");
      }
      ...
      throw new LottaaCryptException("Could not decrypt the data", exc);
    }
  }
  ...
  private Cipher getCipher() throws LottaaCryptException {
    Cipher cipher = cipherWrapper.get();
    if (cipher == null) {
      try {
        cipher = Cipher.getInstance(ALGORITHM);
      } catch (Exception e) {
        throw new LottaaCryptException("Could not get cipher instance", e);
      }
